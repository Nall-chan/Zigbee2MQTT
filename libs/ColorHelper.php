...
trait ColorHelper
{
    ...
    protected function normalizeValueToRange($value, $oldMin = null, $oldMax = null, $newMin = 0, $newMax = 100, $toDevice = true)
    {
        ...
        if ($toDevice) {
            // Prozent -> Gerätewert
            $this->SendDebug(__FUNCTION__, sprintf('Converting %d%% to device value (range %d-%d)',
                $value, $oldMin, $oldMax), 0);
            ...
            // Gerätewert -> Prozent
            $this->SendDebug(__FUNCTION__, sprintf('Converting device value %d (range %d-%d) to percent',
                $value, $oldMin, $oldMax), 0);
        ...
        $this->SendDebug(__FUNCTION__, sprintf('Result: %d (%s)', $result,
            $toDevice ? 'device value' : 'percent'), 0);
        return $result;
    }
